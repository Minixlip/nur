...
export default function LibraryShelf(): React.JSX.Element {
  const { library, loadingLibrary, handleImportNew, openBook, removeBook } =
  ...
  return (
    <div className="p-8 h-full overflow-y-auto">
      ...
      {loadingLibrary ? (
        <div className="text-zinc-400">Loading library...</div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
          {library.map((book) => (
            <div key={book.id} onClick={() => openBook(book)} className="group relative cursor-pointer">
